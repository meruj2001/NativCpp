...
private:
    ...
    const float pedalOffsetThreshold = 0.2;
    const int beginNote = 21;
    const int velocityScale = 128;
public:
    RegressionPostProcessing();

    unordered_map<string, vector<unordered_map<string, float>>>
    outputMapToMidiEvents(unordered_map<string, vector<vector<float>>>);

    unordered_map<string, vector<vector<float>>>
    outputMapToNotePedalArrays(unordered_map<string, vector<vector<float>>>);

    unordered_map<string, vector<vector<float>>>
    getBinarizedOutputFromRegression(vector<vector<float>>, float, int);
...
unordered_map<string, vector<unordered_map<string, float>>>
RegressionPostProcessing::outputMapToMidiEvents(
        unordered_map<string, vector<vector<float>>> outputMap) {
    /*
    ...
    */
    ...
    unordered_map<string, vector<vector<float>>> estOnOffNoteVelsAndPedals = this->outputMapToNotePedalArrays(
            outputMap);
    ...
    */
    ...
    vector<unordered_map<string, float>> estNoteEvents = this->detectedNotesToEvents(
            estOnOffNoteVelsAndPedals["est_on_off_note_vels"]);
    vector<unordered_map<string, float>> estPedalEvents;
    if (estOnOffNoteVelsAndPedals["est_on_off_note_vels"].size() > 0) {
        estPedalEvents = this->detectedPedalsToEvents(
                estOnOffNoteVelsAndPedals["est_pedal_events"]);
    } else {
        estPedalEvents = {};
    }
...
unordered_map<string, vector<vector<float>>> RegressionPostProcessing::outputMapToNotePedalArrays(
        unordered_map<string, vector<vector<float>>> outputMap) {
    /*
...
unordered_map<string, vector<vector<float>>>
RegressionPostProcessing::getBinarizedOutputFromRegression(vector<vector<float>> regOutput,
                                                           float threshold, int neighbour) {
    /*
...
vector<vector<float>> RegressionPostProcessing::outputMapToDetectedNotes(
        unordered_map<string, vector<vector<float>>> outputMap) {
    /*
    ...
    */
    ...
    for (int pianoNote = 0; pianoNote < classesNum; pianoNote++) {
        ...
        estMidiNotes = this->concatVectors(estMidiNotes,
                                           this->concatVectors(pianoNote + this->beginNote,
                                                               estTuplesPerNote.size()));
    }

    vector<float> onsetTimes = this->divideBy(
            this->concatVectors(this->slicing(estTuples, 0), this->slicing(estTuples, 2)),
            this->framesPerSecond);
    vector<float> offsetTimes = this->divideBy(
            this->concatVectors(this->slicing(estTuples, 1), this->slicing(estTuples, 3)),
            this->framesPerSecond);
    vector<float> velocity = this->slicing(estTuples, 4);

    vector<vector<float>> estOnOffNoteVels = this->stack(onsetTimes, offsetTimes, estMidiNotes,
                                                         velocity);

    return estOnOffNoteVels;
}

vector<vector<float>> RegressionPostProcessing::outputMapToDetectedPedals(
        unordered_map<string, vector<vector<float>>> outputMap) {
    /*
...
vector<unordered_map<string, float>>
RegressionPostProcessing::detectedNotesToEvents(vector<vector<float>> estOnOffNoteVels) {
    /*
...
vector<unordered_map<string, float>>
RegressionPostProcessing::detectedPedalsToEvents(vector<vector<float>> estOnOffNoteVels) {
    /*
...
vector<vector<float>>
RegressionPostProcessing::concatVectors(vector<vector<float>> arr1, vector<vector<float>> arr2) {
...
vector<vector<float>>
RegressionPostProcessing::stack(vector<float> onsetTime, vector<float> offsetTime,
                                vector<float> note, vector<float> velocity) {
...
Java_com_example_nativcpp_MainActivity_outputMapToMidiEvents(JNIEnv *env, jobject thisObj, jobject inputHashMap) {
    // Classes from java
    ...
    // Constructors
    jmethodID constructor_ArrayList = env->GetMethodID(ArrayList_Class, "<init>", "()V");    // ArrayList
    jmethodID constructor_HashMap = env->GetMethodID(HashMap_Class, "<init>", "()V");    // HashMap
    jmethodID constructor_Float = env->GetMethodID(Float_Class, "<init>", "(F)V");     // Float
    jmethodID constructor_Double = env->GetMethodID(Double_Class, "<init>", "(D)V"); // Double

    // Methods
    jmethodID getVal_Float = env->GetMethodID(Float_Class, "floatValue", "()F"); // Float
    jmethodID getVal_Double = env->GetMethodID(Double_Class, "doubleValue", "()D"); // Double
    jmethodID add_ArrayListMethod = env->GetMethodID(ArrayList_Class, "add", "(Ljava/lang/Object;)Z"); // ArrayList
    jmethodID get_ArrayListMethod = env->GetMethodID(ArrayList_Class, "get", "(I)Ljava/lang/Object;"); // ArrayList
    jmethodID size_ArrayListMethod = env->GetMethodID(ArrayList_Class, "size", "()I"); // ArrayList
    jmethodID clear_ArrayMethod = env->GetMethodID(ArrayList_Class, "clear", "()V"); // ArrayList
    jmethodID clear_HashMapMethod = env->GetMethodID(HashMap_Class, "clear", "()V");  // HashMap
    jmethodID put_HashMapMethod = env->GetMethodID(HashMap_Class, "put", "(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;");  // HashMap
    jmethodID get_HashMapMethod = env->GetMethodID(HashMap_Class, "get", "(Ljava/lang/Object;)Ljava/lang/Object;");  // HashMap
    jmethodID keySet_HashMapMethod = env->GetMethodID(HashMap_Class, "keySet", "()Ljava/util/Set;");  // HashMap

    unordered_map<string, vector<vector<float>>> input;

    vector<string> keys;
    keys.emplace_back("reg_onset_output");
    keys.emplace_back("reg_offset_output");
    keys.emplace_back("frame_output");
    keys.emplace_back("velocity_output");
    keys.emplace_back("reg_pedal_onset_output");
    keys.emplace_back("reg_pedal_offset_output");
    keys.emplace_back("pedal_frame_output");
    vector<float> tmp;
    vector<vector<float>> tmp2;

    for (const string& key : keys) {
        jobject inputFirstArrayList = env->NewObject(ArrayList_Class, constructor_ArrayList);
        jobject inputArrayList = env->NewObject(ArrayList_Class, constructor_ArrayList);
        inputFirstArrayList = (*env).CallObjectMethod(
                inputHashMap,
                get_HashMapMethod,
                env->NewStringUTF(key.c_str())
                );
        int length = (*env).CallIntMethod(
                inputFirstArrayList,
                size_ArrayListMethod
        );
        for (int i = 0; i < length; i++) {
            inputArrayList = (*env).CallObjectMethod(
                    inputFirstArrayList,
                    get_ArrayListMethod,
                    i
            );
            ...
            for (int j = 0; j < size; j++) {
                jobject val = (*env).CallObjectMethod(
                        inputArrayList,
                        get_ArrayListMethod,
                        j
                );
                double f = (*env).CallDoubleMethod(val, getVal_Double);
                tmp.push_back((float) f);
            }
            tmp2.push_back(tmp);
            tmp.clear();
        }
    ...
    RegressionPostProcessing postProcessing;
    unordered_map<string, vector<unordered_map<string, float>>> cppOutput = postProcessing.outputMapToMidiEvents(input);

    jobject outputMap = env->NewObject(HashMap_Class, constructor_HashMap);
    jobject insideMap = env->NewObject(HashMap_Class, constructor_HashMap);


    for (auto it = cppOutput.begin(); it != cppOutput.end(); ++it) {

        __android_log_print(ANDROID_LOG_DEBUG, "C++", "first");
        jobject arrayListOfMaps = env->NewObject(ArrayList_Class, constructor_ArrayList);
        vector<unordered_map<string, float>> listMap = (*it).second;

        for (unordered_map<string, float> map : listMap) {

            for (auto it2 = map.begin(); it2 != map.end(); ++it2) {
                string _key = it2->first;
                jstring key = (*env).NewStringUTF(_key.c_str());

                float _val = it2->second;
                jobject val = env->NewObject(Float_Class, constructor_Float, _val);
//                string val_cpp = to_string(_val);
//                jstring val = env->NewStringUTF(val_cpp.c_str());

                env->CallObjectMethod(insideMap, put_HashMapMethod, key, val);
            }
            (*env).CallBooleanMethod(arrayListOfMaps, add_ArrayListMethod, insideMap);
        }
        string key = it->first;
        jstring keyNotePedal = (*env).NewStringUTF(key.c_str());
        jobject result = (*env).CallObjectMethod(
                outputMap, put_HashMapMethod, keyNotePedal, arrayListOfMaps
        );
    }
